...
class AuthSecurityTest extends AbstractIntegrationTest {
    ...
    void should_return400_when_salonOwnerRegistersWithoutBusinessName() throws Exception {
        ...
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        // firstName and lastName are required (@NotBlank); include them so the 400 is triggered
        // specifically by the absent businessName, not by the missing name fields.
        String requestBody = String.format(
                "{\"email\":\"%s\",\"password\":\"Str0ngP@ss1!\",\"role\":\"SALON_OWNER\","
                        + "\"firstName\":\"Olena\",\"lastName\":\"Kovalchuk\"}", email);
    ...
    void should_return401_when_revokedRefreshTokenReplayedAfterLogout() throws Exception {
        ...
        restTemplate.postForEntity("/api/v1/auth/register",
                new RegisterRequest(email, "Str0ngP@ss1!", SelfRegistrationRole.CLIENT, "Test", "User", null, null), String.class);
        jdbcTemplate.update("UPDATE users SET email_verified = TRUE WHERE email = ?", email);
